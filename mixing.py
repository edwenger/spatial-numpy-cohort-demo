...
def pairwise_haversine(df):
    ...
    data = df[['Long', 'Lat']].applymap(np.radians).to_numpy()  # N.B. conversion to radians
    lon = data[:, 0]
    lat = data[:, 1]
...
if __name__ == '__main__':
    ...
    def plot_flux(df, M, source, ax):
        ...
        df.plot(kind="scatter", x="Long", y="Lat", s=0.1*np.sqrt(df.population), 
                alpha=0.5, c="flux", norm=LogNorm(), ax=ax)
